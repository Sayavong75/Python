...
# Import du csv : constances poids fille
with open('poids-age-fille-0-60-light.csv', 'r') as csvfile:
...
with open('taille-age-fille-0-60-light.csv', 'r') as csvfile:
    plots= csv.reader(csvfile, delimiter=';')
    for row in plots:
        if (row[0]) != "Month":
            x1.append(int(row[0]))
...
# ----------------------------------------------------------------------------------------------------
# Graphique périmètre crâne fille
x = []
x1 = []
y = []
y1 = []
...
with open('perim-cra-age-fille-0-60-light.csv', 'r') as csvfile:
    plots= csv.reader(csvfile, delimiter=';')
    for row in plots:
        if (row[0]) != "Month":
            x1.append(int(row[0]))
