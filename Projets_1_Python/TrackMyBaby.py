...
# ----------------------------------------------------------------------------------------------------
# genre_nourrisson = input("Entrez le genre de votre nourrison ('g' pour garçon, 'f' pour fille) : ")
genre_nourrisson = ""
while genre_nourrisson != "g" and genre_nourrisson != "f":
    genre_nourrisson = input("Entrez le genre de votre nourrison ('g' pour garçon, 'f' pour fille) : ")

# ----------------------------------------------------------------------------------------------------
# age_nourrisson = input("Veuillez entrer l'âge de votre nourrisson en mois (entre 0 et 60 mois) : ")
age_nourrisson = "Veuillez entrer l'âge de votre nourrisson en mois (entre 0 et 60 mois) : "
while True:
    try:
        age = int(input(age_nourrisson))
        if age < 0 or age > 60:
            raise ValueError
        else:
            age_nourrisson = age
        ...
        age_nourrisson = "------ Erreur de saisie âge ------\nVeuillez entrer une valeur comprise entre 0 et 60 mois : "

# ----------------------------------------------------------------------------------------------------
# poids_nourrisson = input("Veuillez entrer le poids de votre nourrisson en kg : ")
poids_nourrisson = "Veuillez entrer le poids de votre nourrisson en kg : "
while True:
    try:
        poids = float(input(poids_nourrisson))
        if poids < 0:
            raise ValueError
        else:
            poids_nourrisson = poids
            break
    except ValueError:
        poids_nourrisson = "------ Erreur de saisie poids ------\nVeuillez entrer une valeur numérique supérieure à zéro : "

# ----------------------------------------------------------------------------------------------------
# taille_nourrisson = input("Veuillez entrer la taille de votre nourrisson en cm : ")
taille_nourrisson = "Veuillez entrer la taille de votre nourrisson en cm : "
while True:
    try:
        taille = float(input(taille_nourrisson))
        if taille < 0:
            raise ValueError
        else:
            taille_nourrisson = taille
            break
    except ValueError:
        taille_nourrisson = "------ Erreur de saisie taille ------\nVeuillez entrer une valeur numérique supérieure à zéro : "

# ----------------------------------------------------------------------------------------------------
# pericrane_nourrisson = input("Veuillez entrer le périmètre cranien de votre nourrisson en cm : ")
pericrane_nourrisson = "Veuillez entrer le périmètre cranien de votre nourrisson en cm : "
while True:
    try:
        pericrane = float(input(pericrane_nourrisson))
        if pericrane < 0:
            raise ValueError
        else:
            pericrane_nourrisson = pericrane
            break
    except ValueError:
        pericrane_nourrisson = "------ Erreur de saisie périmètre du crâne ------\nVeuillez entrer une valeur numérique supérieure à zéro : "

# ----------------------------------------------------------------------------------------------------
# Poids nourrisson
print("\n------ Check poids nourrisson ------")
if genre_nourrisson == "f":
    poids_mini = low_weights_girls[int(age_nourrisson)]
    poids_maxi = high_weights_girls[int(age_nourrisson)]
    print("La norme de poids pour une fille de " + str(age_nourrisson) + " mois est située entre " + str(poids_mini) + " kg et " + str(poids_maxi) +" kg")
elif genre_nourrisson == "g":
    poids_mini = low_weights_boys[int(age_nourrisson)]
    poids_maxi = high_weights_boys[int(age_nourrisson)]
    print("La norme de poids pour un garçon de " + str(age_nourrisson) + " mois est située entre " + str(poids_mini) + " kg et " + str(poids_maxi) +" kg")
if float(poids_nourrisson) >= float(poids_mini) and float(poids_nourrisson) <= float(poids_maxi):
    print("Le poids de votre nourrisson (" + str(poids_nourrisson) + " kg) est dans la norme !")

# ----------------------------------------------------------------------------------------------------
# Taille nourrisson
print("\n------ Check taille nourrisson ------")
if genre_nourrisson == "f":
    taille_mini = low_heights_girls[int(age_nourrisson)]
    taille_maxi = high_heights_girls[int(age_nourrisson)]
    print("La norme de taille pour une fille de " + str(age_nourrisson) + " mois est située entre " + str(taille_mini) + " cm et " + str(taille_maxi) +" cm")
elif genre_nourrisson == "g":
    taille_mini = low_heights_boys[int(age_nourrisson)]
    taille_maxi = high_heights_boys[int(age_nourrisson)]
    print("La norme de taille pour un garçon de " + str(age_nourrisson) + " mois est située entre " + str(taille_mini) + " cm et " + str(taille_maxi) +" cm")
if float(taille_nourrisson) >= float(taille_mini) and float(taille_nourrisson) <= float(taille_maxi):
    print("La taille de votre nourrisson (" + str(taille_nourrisson) + " cm) est dans la norme !")

# ----------------------------------------------------------------------------------------------------
# Périmètre cranien nourrisson
print("\n------ Check périmètre crânien nourrisson ------")
if genre_nourrisson == "f":
    pericrane_mini = low_skulls_girls[int(age_nourrisson)]
    pericrane_maxi = high_skulls_girls[int(age_nourrisson)]
    print("La norme de périmètre crânien pour une fille de " + str(age_nourrisson) + " mois est située entre " + str(pericrane_mini) + " cm et " + str(pericrane_maxi) +" cm")
elif genre_nourrisson == "g":
    pericrane_mini = low_skulls_boys[int(age_nourrisson)]
    pericrane_maxi = high_skulls_boys[int(age_nourrisson)]
    print("La norme de périmétre crânien pour un garçon de " + str(age_nourrisson) + " mois est située entre " + str(pericrane_mini) + " cm et " + str(pericrane_maxi) +" cm")
if float(pericrane_nourrisson) >= float(pericrane_mini) and float(pericrane_nourrisson) <= float(pericrane_maxi):
    print("Le périmètre crânien de votre nourrisson (" + str(pericrane_nourrisson) + " cm) est dans la norme !")
